...
def draw_bboxes(pred_data, img_name):
    ...
    for bb in bboxes:
        ...
        # Save img
        cv2.imwrite(join(settings.PROCESSED_FOLDER, img_name), img_bb)
...
def draw_mask(pred_data, img_name, mask_alpha: float):
    ...
    # Save img
    cv2.imwrite(join(settings.PROCESSED_FOLDER, img_name), img)
...
def create_heatmaps(height, width, bbs):
    ...
    for bb in bbs:
        ...
        if cls == 0:
            heatmap_obj[y1:y2, x1:x2] = 1.0
        elif cls == 1:
            heatmap_no_obj[y1:y2, x1:x2] = 1.0
...
def create_mask(height, width, heatmaps: tuple, mask_alpha):
    ...
    mask_blur = cv2.GaussianBlur(
        mask, ksize=(51, 51), sigmaX=20, borderType=cv2.BORDER_DEFAULT
    )
